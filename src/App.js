...
import { modalityOptions as fallbackModalityOptions, STUDY_STATUS_OPTIONS } from './configs/const'
...
const suppressNetworkErrorRejections = () => {
  ...
  window.onunhandledrejection = event => {
    try {
      ...
      if (
        reason &&
        (!reason.response || reason.isNetworkError || /Network Error/i.test(reason?.message || ''))
      ) {
        event.preventDefault()
        return true
      }
...
const App = props => {
  ...
  useEffect(() => {
    ...
    const updateAdvanceFilter = ({ advanceFilter }) => {
      ...
      if (advanceFilter === 1) {
        if (!access.find(a => a.action === 'manage' && a.subject === 'filter-listings')) {
          access.push({ action: 'manage', subject: 'filter-listings' })
        }

        if (
          userData.role === ROLES.ClinicUser &&
          access.find(a => a.action === 'manage' && a.subject === 'filter-listings')
        ) {
          access.splice(
            access.findIndex(a => a.action === 'manage' && a.subject === 'filter-listings'),
            1
          )
        }
    ...
    socket.on(`completedPatientEditProcess_${userData?._id}`, onCompletedPatientEditProcessEvent)

    return () => {
      socket.off(`completedPatientEditProcess_${userData?._id}`, onCompletedPatientEditProcessEvent)
      socket.off(
        `updateAdvanceFilter_${userData?.parent_user ? userData?.parent_user : userData?._id}`,
        updateAdvanceFilter
      )
  ...
  const pagesNeedingFilterData = [
    '/study-list',
    '/settings/filter_listings',
  ]
  
  // Track previous pathname to detect route changes
  const [previousPathname, setPreviousPathname] = useState(window.location.pathname)
  ...
  const fetchModalitiesFromOrthanc = async () => {
    ...
    try {
      ...
      if (isSuccess && !isErrorBody) {
        ...
        if (Array.isArray(data)) {
          ...
          console.log('[Modality] Response was object — normalizing keys to AET. Object keys:', Object.keys(data))
          const names = new Set()
          ...
          console.log('[Modality] After object→AET normalize, raw array:', JSON.stringify(raw))
        }
      }
      console.log('[Modality] Final raw length:', raw?.length, '| full raw array:', JSON.stringify(raw))
    ...
    console.log('[Modality] Using fallback list (configs/const.js), count:', fallbackModalityOptions?.length ?? 0)
  ...
  useEffect(() => {
    const fetchFilterData = async () => {
      ...
      console.log(
        ...
        'hasToken=', !!loginState?.accessToken,
        'hasUserData=', !!userDataRedux?._id
      )
      ...
      console.log('[Modality] Dispatching to Redux, count:', modalityOptionsFromApi?.length ?? 0)
