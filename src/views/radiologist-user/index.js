...
const RadiologistUser = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(
    localStorage.getItem('radiologistuserrow')
      ? JSON.parse(localStorage.getItem('radiologistuserrow'))
      : 7
  )
  ...
  const getUpdatedData = async () => {
    ...
    console.log('Fetching radiologist users with params:', params)
    axios
      ...
      .then((doc) => {
        ...
        if (doc.data.numberOfRecord >= parseInt(userData.radiologist || 0)) {
          setLimitReach(true)
        } else {
          setLimitReach(false)
        }
  ...
  const addNewUser = (requestData) => {
    ...
    console.log('Creating radiologist user with data:', requestData)
    showLoadingAlert()
    axios
      ...
      .then((doc) => {
        ...
        handleModal()
        hideLoadingAlert()
        showSuccessAlert('Radiologist Added Successfully!')
  ...
  const limitHandler = () => {
    showInfoAlert('You have reached the limit to create new radiologist.')
  }
              ...
              <X
                ...
                onClick={() => {
                  handleConfirm(
                    row._id,
                    DeactivationHandler,
                    'You want to deactivate this Radiologist User?',
                    'Yes, Deactivate!'
                  )
                }}
              />
              ...
              <Check
                ...
                onClick={() => {
                  handleConfirm(
                    row._id,
                    ActivationHandler,
                    'You want to activate this Radiologist User?',
                    'Yes, Activate!'
                  )
                }}
              />
        ...
        <Col sm="12">
          <Card>
            <CardHeader className="flex-md-row flex-column align-md-items-center align-items-center border-bottom">
              <CardTitle tag="h4">Radiologist User</CardTitle>
